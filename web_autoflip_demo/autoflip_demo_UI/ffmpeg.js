...
class PooledObjectFileSystem {
  ...
  reserveObjectPoolMemory(startingPoolSize, expectedSizeOfObjectsInBytes) {
    this.pooledObjects_ = new Array(startingPoolSize).fill(new ArrayBuffer(expectedSizeOfObjectsInBytes));
  ...
  mount(_) {
    if (this.pooledObjects_) {
      ...
      this.pooledObjectInUse = new Array(this.pooledObjects_.length).fill(false);
    }
  ...
  write(stream, buffer, offset, length, position, canOwn) {
    ...
    if (buffer instanceof ArrayBuffer) throw new Error('The data buffer should be a typed array view');
    ...
    if (buffer.subarray && (!node.contents || node.contents.subarray)) {
      ...
      if (canOwn) {
        if (!(position === 0)) throw new Error('canOwn must imply no weird position inside the file');
...
class WasmModule {
  ...
  createFFmpegWasmInstance() {
    if (this.wasmRunning_) {
      return Promise.reject('Failed to create ffmpeg wasm instance due to one already running!');
  ...
  reserveObjectPoolMemory(startingPoolSize, expectedSizeOfObjectsInBytes) {
    this.poolFS_.reserveObjectPoolMemory(startingPoolSize, expectedSizeOfObjectsInBytes);
  }
    ...
    for (let i = 0; i < files.length; i++) {
      switch (files[i].type) {
        ...
        case FfmpegFileType.DEFAULT:
          this.FS_.createDataFile('/', files[i].name, files[i].data, kCanRead, kCanWrite);
          break;
    ...
    if (fileHandle && fileHandle.object && fileHandle.object.contents) {
      for (const fileName in fileHandle.object.contents) {
        if (fileHandle.object.contents.hasOwnProperty(fileName)) {
          buffers.push({
            name: fileName,
            data: new Uint8Array(fileHandle.object.contents[fileName].contents).buffer,
          });
        }
      }
    }
