...
let dbFFmpeg: IDBDatabase;
const requestF = indexedDB.open('auto-flip', 1);
requestF.onerror = (event: Event) => {
  console.error('FFMPEG: Failed to load indexeddb');
  throw new Error(String(event));
};
requestF.onsuccess = (event) => {
  dbFFmpeg = (event.target as IDBOpenDBRequest).result;
};
...
onmessage = function (e: MessageEvent): void {
  ...
  function addSectionFramestoIndexDB(
    ...
    let transaction: IDBTransaction = dbFFmpeg.transaction(
      ['decodedFrames'],
      'readwrite',
    );
