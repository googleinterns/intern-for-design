...
::mediapipe::Status SceneCropper::CropFrames(
  ...
  for (int i = 0; i < num_scene_frames; i++) {
    ...
    const int top = top_static_border_size - (scene_frame_xforms[i].at<float>(1, 2));
    RET_CHECK_LE(crop_height, frame_height_ - bottom_static_border_size - top)
      << "Crop height exceeds border. Border detection is wrong.";
    crop_from_location->push_back(
        cv::Rect(left, top, crop_width, crop_height));
  }
