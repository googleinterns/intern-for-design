...
constexpr char kInputSpeakerBoundaries[] = "SPEAKER_BOUNDARIES";
...
::mediapipe::Status SceneCroppingCalculator::GetContract(
  ...
  if (cc->Inputs().HasTag(kInputSpeakerBoundaries)) {
    cc->Inputs().Tag(kInputSpeakerBoundaries).Set<bool>();
  }
...
::mediapipe::Status SceneCroppingCalculator::Process(
  ...
  // Processes a scene when shot boundary, speaker boundary or buffer is full.
  ...
  if (cc->Inputs().HasTag(kInputSpeakerBoundaries) &&
      !cc->Inputs().Tag(kInputSpeakerBoundaries).Value().IsEmpty()) {
    is_end_of_scene = is_end_of_scene || cc->Inputs().Tag(kInputSpeakerBoundaries).Get<bool>();
  }
